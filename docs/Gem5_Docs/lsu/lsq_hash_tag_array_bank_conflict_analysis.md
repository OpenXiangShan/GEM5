...
DLB 使用同一最终二进制，在显式 `8B / SetDiv=1 / HTA off / prefetch off` 下做了
开关 A/B：

| 模式 | simInsts | IPC | `bankConflictTimes` | `BankConflictReplay` | DLB bank query / hit | DLB insertions |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| DLB off | 3,151,499 | 2.241830 | 7,916 | 7,875 | 0 / 0 | 0 |
| DLB on | 3,151,499 | 2.241633 | 20 | 19 | 345,839 / 343,973 | 1,411 |

上表是本次 8B 验证；此前的 1 KiB L1D CoreMark invalidation smoke 是独立的 2B
几何实验，记录了 `dlbL1EvictInvalidations=1,640`，仅用于覆盖
`BaseCache::invalidateBlock()` 通知路径，不能当作本次 8B A/B 的结果。正常 8B
CoreMark A/B 的 L1-evict invalidation 为 0。这些 smoke 只验证 DLB update、bypass
和 invalidation 路径的活性，不用于推导 SPEC 收益。2B smoke 只覆盖 L1-evict
invalidation；SPEC 中
...
| Overall score/GHz | 19.687403 | 19.717179 (+0.151%) | 20.072955 (+1.958%) | 19.801487 (+0.579%) | 19.976591 (+1.469%) |
| Dynamic-instruction weighted IPC | 6.674857 | 6.685802 (+0.164%) | 6.822081 (+2.206%) | 6.715965 (+0.616%) | 6.779958 (+1.575%) |
...
| Overall score/GHz | 19.687403 | 19.976591 | +1.469% |
...
为准。直接相对 8B baseline，DLB 的 `BankConflictReplay` 最大降幅出现在 `lbm`
(-94.223%)、`hmmer_nph3` (-94.058%)、`hmmer_retro` (-93.784%)、
`astar_rivers` (-90.697%) 和 `astar_biglakes` (-89.656%)；对应 score 的主要收益是
...
- 这份比较不包含 Dcache bank/set 变化的面积、能耗、时序或 RTL 实现成本。HTA 结果
  ...
  `38457bc541` 上 `DLB=false, 8B, SetDiv=1, HTA=false` 的同提交 SPEC control；本地
  8B CoreMark off/on 证明功能路径，但不能消除这个跨提交的 SPEC 归因限制。
