...
每一个 GEM5 candidate（包括 manual 确认后的验证重跑和 autonomous 迭代）都必须保留新的 lifetime DB，并重新运行 `ClockAnalysis.py -P gem5 --inter-gap`；只有 IPC/stats 而没有 candidate trace 的重跑不能用于指令级对齐结论。

## RTL baseline（最多一次）

`WITH_CHISELDB=1` 只使 emu 支持 ChiselDB；不会自动选择 lifetime 表。运行前确认 `"<RTL_WORKTREE>/build/emu" --help` 包含 `--dump-select-db`。若没有该选项，停止并检查当前 RTL worktree/构建产物，不能把普通 DB 当作 lifetime trace。
...
"<RTL_WORKTREE>/build/emu" \
  ...
  --dump-db --dump-select-db lifetime \
...
`--dump-select-db lifetime` 是 lifetime trace 的必需运行参数；`--dump-db` 与 `--db-path` 同时保留，分别启用 dump 和固定产物位置。完成后必须按 [trace-and-inter-gap](trace-and-inter-gap.md) 检查 DB 的 `lifetime` 表及所需 schema，缺失时本次 RTL trace 不合格。共享 `perf_trigger/main.py` 只能转发 `--dump-db`，不能转发 `--dump-select-db`：若必须通过该入口运行，先用 `<RTL_WORKTREE>/scripts/patch_chiseldb_lifetime.py` 将生成的 ChiselDB 默认选择改为 `lifetime`，再重编译；否则改用上述 emu 直接命令。RTL 任务可能运行数小时：记录 PID/job、命令、日志和预期 DB 后交还控制权，不持续轮询。自动模式只有在缺少合格 baseline 时允许执行这一次 RTL 运行，之后只重跑 GEM5。
