...
## Goal
Imported from OpenSpec change `add-fdip-icache-prefetch` at `.worktrees/impl-control-pc-views/openspec/changes/add-fdip-icache-prefetch`.
在当前 decoupled frontend 中，Fetch 以 FTQ 为取指驱动（每个 FTQ entry 对应一次取指窗口，
在本仓库实现中 66B fetch 可能跨 2 条 cacheline request），ICache miss 仍然会直接导致前端停顿。
...
1. **RTL-aligned 的 ICache 预取引擎（FDIP engine）**
   ...
   - 行为：基于 FTQ 的 **`prefetchPtr/fetchPtr` 语义** 驱动预取：
     ...
     - FDIP 每次只处理 `prefetchPtr` 指向的一个 FTQ entry，并按 cacheline 粒度向 ICache 发起 inst-fetch 预取；
...
## Technical Notes
- Source workspace: `impl-control-pc-views`
- Source OpenSpec root: `.worktrees/impl-control-pc-views/openspec`
- Source change dir: `.worktrees/impl-control-pc-views/openspec/changes/add-fdip-icache-prefetch`
- Imported checklist progress: 0/42
