...
## Goal
Imported from deduplicated OpenSpec change `add-fdip-icache-prefetch`.
Canonical source: `.worktrees/fdip-phase2-xsdev/openspec/changes/add-fdip-icache-prefetch`.
...
## Technical Notes
- Canonical source workspace: `fdip-phase2-xsdev`
- Canonical OpenSpec root: `.worktrees/fdip-phase2-xsdev/openspec`
- Canonical change dir: `.worktrees/fdip-phase2-xsdev/openspec/changes/add-fdip-icache-prefetch`
- Canonical checklist progress: 38/116
...
- Base branch: `impl-control-pc-views`
- Worktree: `.worktrees/fdip-phase2-xsdev`

## Isolation Contract
- Only edit inside `.worktrees/fdip-phase2-xsdev`.
- Do not touch the Wave-1 research-framework branch/worktree.
- Do not touch the Wave-1 SRAM-study branch/worktree.
- Do not migrate optional RTL-alignment experiments from `fdip-rtl-align-xsdev` unless they are strictly required to close current P0/P1 gaps.
...
1. Inspect the dirty worktree and group changes into reviewable slices.
2. Remove or classify `.tmp/`.
3. Re-check actual-coverage semantics and cross-boundary 4B control-tail handling.
4. Refresh the remaining focused smoke / witness gaps.
5. Update docs and validation artifacts only after commit slicing is clear.

## Stop Rules
- Stop if the dirty worktree contains unrelated edits that cannot be cleanly separated.
- Stop if a proposed fix starts pulling in optional Phase-2/Phase-3 RTL modeling work.
