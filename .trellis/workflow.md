...
6. [!] **Don't** execute `git commit` unless the repository owner explicitly
   authorizes the AI to do so for the current session
...
python3 ./.trellis/scripts/import_openspec_changes.py  # Import active OpenSpec changes into root Trellis tasks, dedupe by change_id, and keep variant metadata
