...
- upstream 参考：`upstream/develop` = `eb344eb5ee91e86b9fa0358e653d3b4c738f3b4e`
- upstream 最近 tag：`v25.0.0.1`
- upstream 相对基线新增 commit：**4531**（merge commits: 604）
- 与 `origin/xs-dev` 的对比：origin-only=1889，upstream-only=4531

## 复现步骤（可重复）

```bash
git fetch --all --prune
git merge-base origin/xs-dev upstream/develop
git rev-list --left-right --count origin/xs-dev...upstream/develop
...
- 时间跨度（commit date）：`2022-06-29T12:40:28+00:00` -> `2026-01-16T11:08:25-08:00`
- 贡献者（author 去重）：231
