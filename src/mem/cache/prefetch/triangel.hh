...
#include "mem/cache/prefetch/bloom.hh"
...
class TriangelHashedSetAssociative : public SetAssociative
{
  ...
  public:
    int ways;
    int max_ways;
    TriangelHashedSetAssociative(const TriangelHashedSetAssociativeParams &p) : SetAssociative(p), ways(0), max_ways(8)
    {
    }
...
class Triangel : public Queued
{
    ...
    BaseTags *cachetags;            // Pointer to the cache tags
    const unsigned cacheDelay;      // Delay for cache access
    ...
    struct TrainingUnitEntry : public TaggedEntry
    {
        ...
        TrainingUnitEntry()
            : lastAddress(0),
              ...
              local_timestamp(0),
              reuseConfidence(4, 8),
              patternConfidence(4, 8),
              highPatternConfidence(4, 8),
              replaceRate(4, 8),
              hawkConfidence(4, 8),
              lastAddressSecure(false),
              lastLastAddressSecure(false),
        ...
        void invalidate() override
        {
            ...
            lastAddress = 0;
            lastLastAddress = 0;
            // local_timestamp=0; //Don't reset this, to handle replacement and still
            // give contiguity of timestamp
  ...
  public:
    struct SizeDuel
    {
        ...
        int checkAndInsert(Addr addr, bool should_pf)
        {
            ...
            for (int x = 0; x < cacheMaxAssoc; x++) {
                if (addr == cacheAddrs[x]) {
                    ...
                    for (int y = 0; y < cacheMaxAssoc; y++) {
                        if (cacheAddrTick[x] > cacheAddrTick[y])
                            index--;
                        assert(index >= 0);
                    }
                ...
                if (should_pf && addr == temporalAddrs[x]) {
                    ...
                    int index = cacheMaxAssoc - 1;
                    for (int y = 0; y < cacheMaxAssoc; y++) {
                        if (temporalAddrTick[x] > temporalAddrTick[y])
                            index--;
                        assert(index >= 0);
                    }
            ...
            if (!foundInCache) {
                ...
                int idx = -1;
                for (int x = 0; x < cacheMaxAssoc; x++) {
                    if (cacheAddrTick[x] < oldestTick) {
                        idx = x;
                        oldestTick = cacheAddrTick[x];
                    }
                }
                assert(idx >= 0);
                cacheAddrs[idx] = addr;
            ...
            if (!foundInTemp && should_pf && (((addr / (setMask + 1)) % temporalModMax) == temporalMod)) {
                ...
                int idx = -1;
                for (int x = 0; x < cacheMaxAssoc; x++) {
                    if (temporalAddrTick[x] < oldestTick) {
                        idx = x;
                        oldestTick = temporalAddrTick[x];
                    }
                }
                assert(idx >= 0);
    ...
    struct Hawkeye
    {
        ...
        Hawkeye(uint64_t mask, bool history) : iteration(0), set(0), setMask(mask) { reset(); }
    ...
    /** Sample unit entry, tagged by data address, stores PC, timestamp, next
     * element **/
    struct SampleEntry : public TaggedEntry
    {
        ...
        void invalidate() override { TaggedEntry::invalidate(); }
  ...
  protected:
    struct TriangelStats : public statistics::Group
    {
        TriangelStats(statistics::Group *parent);
