...
inline static int
test_bit_set_bit(unsigned char *buf, unsigned long int bit, int set_bit)
{
...
static int
bloom_check_add(struct bloom *bloom, const void *buffer, int len, int add)
{
    if (bloom->ready == 0) {
...
int
bloom_init(struct bloom *bloom, int entries, double error)
{
...
int
bloom_init2(struct bloom *bloom, unsigned int entries, double error)
{
...
int
bloom_check(struct bloom *bloom, const void *buffer, int len)
{
...
int
bloom_add(struct bloom *bloom, const void *buffer, int len)
{
...
void
bloom_print(struct bloom *bloom)
{
    printf("bloom at %p\n", (void *)bloom);
    if (!bloom->ready) {
        printf(" *** NOT READY ***\n");
...
int
bloom_reset(struct bloom *bloom)
{
    if (!bloom->ready)
        return 1;
...
int
bloom_save(struct bloom *bloom, char *filename)
{
...
int
bloom_load(struct bloom *bloom, char *filename)
{
    int rv = 0;
...
int
bloom_merge(struct bloom *bloom_dest, struct bloom *bloom_src)
{
