#ifndef __MEM_CACHE_XS_L3_L3_MAIN_PIPE_HH__
#define __MEM_CACHE_XS_L3_L3_MAIN_PIPE_HH__
...
class L3MainPipe
{
  public:
     ...
     * Eg. L2MSHR need DirWrite when hit in L2 cache.
     * @param pkt The packet to check.
     * @param source The task source.
     * @return The extra resources that are required for the task.
     */
    L3PipelineResources getExtraResources(PacketPtr pkt, L3TaskSource source) const;
     ...
     */
    bool setBlockByDir(PacketPtr pkt, L3TaskSource source) const;
     ...
     */
    bool hasMCP2Stall(L3PipelineResources resource, PacketPtr pkt) const;
     ...
     * @return True if the resource is available, false otherwise.
     */
    bool hasDirSramStall(L3PipelineResources resource, PacketPtr pkt) const;
     ...
     * @return True if the resource is available, false otherwise.
     */
    bool isResourceAvailable(L3PipelineResources resource, PacketPtr pkt) const;
     ...
     */
    bool isTaskAvailable(PacketPtr pkt, L3TaskSource source) const;
     ...
     */
    void buildTask(PacketPtr pkt, L3TaskSource source);
     ...
     */
    inline L3PipelineResources getPipelineResources(PacketPtr pkt, L3TaskSource source) const;
  ...
  private:
    struct PipelineTask
    {
        ...
        PacketPtr pkt;
        Addr addr;
        PipelineTask(L3TaskSource source, PacketPtr pkt) : source(source), pkt(pkt), addr(0) {
          if (pkt) {
            addr = pkt->getAddr();
          }
        }
    ...
    std::deque<PipelineTask> scoreboardTasks; // Bitmask of L3TaskSource
    std::unordered_map<L3TaskSource, L3PipelineResources> taskResourceMap;
    Cycles cur_cycle;
...
#endif // __MEM_CACHE_XS_L3_L3_MAIN_PIPE_HH__
