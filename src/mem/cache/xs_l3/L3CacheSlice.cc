...
L3CacheSlice::processResponses()
{
    ...
    if (pkt &&
        reqArb.arbitrate(L3TaskSource::L3MSHRGrant, curCycle()) &&
        mainPipe.isTaskAvailable(pkt, L3TaskSource::L3MSHRGrant))
    {
        DPRINTF(L3CacheSlice, "Building L3 MSHR grant task for addr: %#x\n", pkt->getAddr());
        mainPipe.buildTask(pkt, L3TaskSource::L3MSHRGrant);
...
{
    panic("L3CacheSlice should not receive resp retry from inner L2");
}
...
L3CacheSlice::cpuSidePortRecvTimingReq(PacketPtr pkt)
{
    ...
    if (!pkt->needsResponse() && !is_prefetch) {
        ...
        bool success = innerCpuPortSendTimingReq(pkt, L3TaskSource::L2WQ);
        if (!success) {
            DPRINTF(L3CacheSlice, "Inner cache busy, rejecting WQ request from CPU side\n");
    ...
    L3TaskSource source = is_prefetch ? L3TaskSource::L3PF : L3TaskSource::L2MSHR;
...
L3CacheSlice::trySendFromBuffer()
{
    if (requestBuffer.empty() && !inner_cache_blocked && pending_l2_retry) {
        DPRINTF(L3CacheSlice, "No pending request in buffer, send retry to L1\n");
    ...
    if (!innerCpuPortSendTimingReq(pkt, L3TaskSource::L2MSHR)) {
    ...
    if (requestBuffer.empty() && !inner_cache_blocked && pending_l2_retry) {
        DPRINTF(L3CacheSlice, "Buffer empty, sending retry to L1\n");
        pending_l2_retry = false;
        cpu_side_port.sendRetryReq();
    }
}
...
bool
L3CacheSlice::innerCpuPortSendTimingReq(PacketPtr pkt, L3TaskSource source)
{
