...
| 数据流主线 | `L2Wrapper(Packet)->Request -> CHIBridge(Req Flit) -> MeshNode(XY+VOQ+RR) -> FakeL3(HN 转换) -> DDRWrapper(DRAMsim3)`，返回路径反向 |
...
- **device/**：具体设备/适配器。
  ...
  - 其他：`HNF.hh` 为 FakeL3 声明头，当前以 `FakeL3` 作为简化 HN 行为实现。
- **TopoSys/**：组合系统。
...
| 并发语义 | `L2Wrapper` outstanding 以地址为 key，限制同地址并发请求 | 后续改为 txn-key 或 `(addr, txn)` 复合 key |
