...
namespace xsCHI
{
    L2Wrapper::WrapperStats::WrapperStats(L2Wrapper *parent)
        : statistics::Group(parent, "addr_observe"),
          ADD_STAT(observed_req_count, statistics::units::Count::get(),
                   "Observed cacheable requests entering xsCHI L2Wrapper"),
    ...
    void
    L2Wrapper::recordObservedAddress(Addr addr)
    {
        if (!hasObservedAddr) {
    ...
    Addr
    L2Wrapper::remapShadowAddr(size_t shadowIdx, Addr addr)
    {
    ...
    void
    L2Wrapper::mirrorReqToShadows(const ReqPtr &req)
    {
        if (!shadowEnabled) {
            return;
        }
    ...
    void
    L2Wrapper::recvShadowReadResp(size_t shadowIdx, ReqPtr &req)
    {
