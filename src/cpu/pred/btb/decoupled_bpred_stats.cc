...
DecoupledBPUWithBTB::DBPBTBStats::DBPBTBStats(
    ...
    ADD_STAT(h2pTableCandidates, statistics::units::Count::get(),
             "conditional branches classified as H2P by the PC-only table"),
    ADD_STAT(h2pCandidates, statistics::units::Count::get(),
             "conditional branches accepted after H2P confidence filtering"),
    ADD_STAT(h2pConfidenceRejected, statistics::units::Count::get(),
             "PC-H2P candidates rejected by the TAGE confidence filter"),
    ADD_STAT(h2pConfidenceMissing, statistics::units::Count::get(),
             "PC-H2P candidates rejected because TAGE confidence was missing"),
    ADD_STAT(h2pTableTruePositive, statistics::units::Count::get(),
             "PC-only H2P-marked conditional branches that mispredicted"),
    ADD_STAT(h2pTableFalsePositive, statistics::units::Count::get(),
             "PC-only H2P-marked conditional branches that were correct"),
    ADD_STAT(h2pTableFalseNegative, statistics::units::Count::get(),
             "PC-only H2P-unmarked conditional branches that mispredicted"),
    ADD_STAT(h2pTableTrueNegative, statistics::units::Count::get(),
             "PC-only H2P-unmarked conditional branches that were correct"),
    ADD_STAT(h2pTableCoverage, statistics::units::Ratio::get(),
             "PC-only H2P coverage: TP / (TP + FN)",
             h2pTableTruePositive /
                 (h2pTableTruePositive + h2pTableFalseNegative)),
    ADD_STAT(h2pTablePrecision, statistics::units::Ratio::get(),
             "PC-only H2P precision: TP / (TP + FP)",
             h2pTableTruePositive /
                 (h2pTableTruePositive + h2pTableFalsePositive)),
    ADD_STAT(h2pTableWastage, statistics::units::Ratio::get(),
             "PC-only H2P wastage: FP / (TP + FP)",
             h2pTableFalsePositive /
                 (h2pTableTruePositive + h2pTableFalsePositive)),
    ...
    const FetchTarget &target, const PreparedUpdate &update)
{
    ...
    const bool miss_predicted = update.outcome.controlMispred;
    // Track indirect mispredictions
    if (miss_predicted && update.outcome.branch.isIndirect) {
    ...
    if (target.commitInstNum >= 0 && target.commitInstNum <= maxInstsNum) {
        ...
        if (target.commitInstNum == 1 &&
            update.outcome.valid && update.outcome.branch.isUncond()) {
    ...
    if (update.outcome.controlMispred) {
        // Record mispredict pair (start PC, branch PC)
        auto find_it = topMispredicts.find(
            std::make_pair(target.startPC, update.outcome.branch.pc));
        if (find_it == topMispredicts.end()) {
            topMispredicts[
                std::make_pair(target.startPC, update.outcome.branch.pc)] = 1;
        } else {
            find_it->second++;
        }
