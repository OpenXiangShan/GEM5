...
DecoupledBPUWithFTB::makeNewPrediction(bool create_new_stream)
{
    ...
    entry.predEndPC = finalPred.ftbEntry.fallThruAddr; // TODO: if taken, taken snpc
    // TODO: if taken, set branch info with corresponding ftb slot
    // entry.predBranchInfo =

    // TODO: update s0PC
    if (finalPred.valid) {
        if (taken) {
            // get target from taken slot
            entry.predEndPC = finalPred.ftbEntry.targetAddr;
            s0PC = finalPred.ftbEntry.targetAddr;
        } else {
            s0PC = finalPred.ftbEntry.fallThruAddr;
        }
    } else {
        s0PC = s0PC + 32; // default: +PredictWidth
    }
