...
def SendMsg(msg: str):
    if cfgfile['global']['debug_mode'] == 'true':
        print(msg)
    elif cfgfile['global']['debug_mode'] == 'false':
        pass
...
def startMain(work, log_dir: str, etcArg):
    ...
    other.write(
        '**********runTime:{0}h:{1}m:{2}s**********\n'.format(
            tupTime // 3600, (tupTime % 3600)//60, (tupTime % 3600) % 60))
    # start except-task
    if not (post and post.returncode == 0):
        ...
        exce = subprocess.run(args=task[3], shell=True, stdout=other,
                              stderr=subprocess.STDOUT, stdin=None, check=False, encoding='utf-8')
        SendMsg(
            """error in:[work:{0}][log:{1}]""".format(work[0], log_))
        other.close()
        utils.tpool_free(C)
        return False
    other.close()
...
def Wrun_multi(log_dir, etcArg):
    ...
    for work in work_items:
        ...
        results.append(pool.apply_async(
            startMain,
            (work, log_dir+'/'+work[0],  # 添加自定义特殊变量
             dict({'tid': cnt, 'random_int': random_int}, **etcArg))))
        cnt += 1
            ...
            results.append(
                pool.apply_async(
                    ...
                    (work,
                     ...
                     dict({'workname' : work[0], 'tid': cnt, 'random_int': random_int, 'binfile': files[i]}, **etcArg))))  # 添加自定义特殊变量
            cnt += 1
            if cnt >= int(cfgfile['iteration']['max_process']):
                cnt = 0
            time.sleep(0.5)
...
def iteration():
    ...
    if finished0:
        ...
        if cfgfile['iteration']['working_mode'] == 'multi':
            ...
            exit(1)
        if not finished1:
            error_msg += 'found {0} works error'.format(len(runErr_works))
    else:
