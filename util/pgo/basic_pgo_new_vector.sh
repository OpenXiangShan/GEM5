...
CC=clang CXX=clang++ scons build/RISCV/gem5.fast -j $build_threads --gold-linker --pgo-prof --rvv-impl=simple
check $?
...
# build gem5 with pgo instrumentation
CC=clang CXX=clang++ scons build/RISCV/gem5.fast -j $build_threads --gold-linker --pgo-use=llvm-pgo/gem5_single.profdata --rvv-impl=simple
check $?
